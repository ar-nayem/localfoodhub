...
/** Shop signup form, shared by the customer site's /apply and the Business app's
 * /vendor/apply so the two can't drift. `signInHref` is where an applicant goes next —
 * each hostname has its own sign-in. */
export function ApplyForm({ signInHref, business = false }: { signInHref: string; business?: boolean }) {
  ...
  if (done) {
    return (
      <main className="mx-auto flex min-h-[70vh] max-w-sm flex-col items-center justify-center px-6 text-center">
        ...
        <Button className="mt-5" onClick={() => router.push(signInHref)}>
          Go to sign in
        </Button>
      </main>
    );
  ...
  return (
    <main className="mx-auto max-w-md px-4 py-10">
      <div className="mb-6 flex justify-center">
        <Logo variant={business ? "business" : "customer"} />
      </div>
      ...
      <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
        ...
        <p className="text-center text-sm text-muted-foreground">
          Already registered?{" "}
          <Link href={signInHref} className="font-medium text-primary">
            Sign in
          </Link>
        </p>
      </form>
